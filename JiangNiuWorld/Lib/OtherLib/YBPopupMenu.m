...
{
    CGRect absoluteRect = [view convertRect:view.bounds toView:BXKeyWindow];
...
- (void)show {
    ...
    [BXKeyWindow addSubview: _bgView];
    [BXKeyWindow addSubview: self];
    YBPopupMenuCell *cell = [self getLastVisibleCell];
    cell.isShowSeparator = NO;
    self.layer.affineTransform = CGAffineTransformMakeScale(0.1, 0.1);
    [UIView animateWithDuration: 0.25 animations:^{
...
{
    CGPoint aPoint;
    if (CGRectGetMaxY(self.frame) > SCREEN_HEIGHT) {
...
- (void)setArrowPointingWhere: (CGPoint)anchorPoint
{
    ...
    if (maxX > SCREEN_WIDTH - 10) {
        self.x = SCREEN_WIDTH - 10 - self.width;
    }else if (minX < 10) {
        self.x = 10;
    }
